...
from .logging_config import thread_local
...
def process_drive(
    ...
    try:
        ...
        if run_sync_op:
            ...
            with db.transaction():
                sync_result = scanner.sync_directory(target_path, db)
            logger.info(
                f"Sync completed - Scanned: {sync_result.files_scanned}, "
        ...
        if run_scrub_op:
            ...
            scrub_result = scanner.scrub_files(db, scrub_percentage, scrub_frequency)
            logger.info(
                f"Scrub completed - Validated: {scrub_result.files_validated}, "
