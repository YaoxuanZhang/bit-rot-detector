...
// newTestStore returns an in-memory config.Store with default config and the
// given target dir so Load-based validation succeeds.
func newTestStore(dir string) *config.Store {
...
// When no config store is injected, GET /api/settings returns defaults.
srv := newTestServer(t, t.TempDir())
...
dir := t.TempDir()
srv := newTestServer(t, dir)
srv.WithConfigStore(newTestStore(dir))
...
func TestGetConfig_NoStore_ReturnsDefaults(t *testing.T) {
srv := newTestServer(t, t.TempDir())
...
if got["disk_thresholds"] == nil {
t.Error("expected disk_thresholds key in config response")
}
}

func TestGetConfig_WithStore(t *testing.T) {
dir := t.TempDir()
srv := newTestServer(t, dir)
srv.WithConfigStore(newTestStore(dir))
...
smtp, _ := got["smtp"].(map[string]interface{})
if smtp != nil {
if _, has := smtp["username"]; has {
t.Error("SMTP username must not appear in /api/config response")
}
if _, has := smtp["password"]; has {
t.Error("SMTP password must not appear in /api/config response")
}
}
}

func TestPostConfig_NoStore_Returns503(t *testing.T) {
srv := newTestServer(t, t.TempDir())
...
func TestPostConfig_UpdateScrubPercentage(t *testing.T) {
dir := t.TempDir()
srv := newTestServer(t, dir)
srv.WithConfigStore(newTestStore(dir))

body := `{"scrub_percentage":10.0}`
